...
public class ProcessQualityGates {
	...
	public static QualityGateReport processCICDInputs(ApiClient apiClient, RegressionInput input, boolean newEvents,
			boolean resurfacedEvents, String regexFilter, int topIssuesVolume, boolean countGate, PrintStream printStream, boolean verbose) {
		...
		if (deploymentStart == null) {
			throw new IllegalStateException("Deployment name " + input.deployments + " not found. Please ensure your collector and Jenkins configuration are pointing to the same enviornment.");
		}
