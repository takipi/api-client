...
public class EventsInput extends BaseEventVolumeInput {
	...
	@Param(type=ParamType.String, advanced=false, literals={},
			description = "A comma delimited list of attributes defined in: https://doc.overops.com/reference#get_services-env-id-events-event-id " + 
				 ...
				 "id = OverOps assigned ID for this unique event \n" +
				 "summary = short text summary of the event \n" +
				 "type = the type of event: Uncaught Exception, Swallowed Exception, Logged Error,.. \n" +
				 "name = The type of exception in case of exception (i.e. NullPointerException) otherwise the type\n" +
				 ...
				 "entry_point = the first frame within the event's call stack which falls within app code (e.g. myServlet.goGet()). \n" +
	...
	public static final List<String> FIELDS = Arrays.asList(new String[] {
			ENV_ID, ID, SUMMARY, MESSAGE, TYPE, FIRST_SEEN, ERROR_LOCATION, ERROR_ORIGIN,
